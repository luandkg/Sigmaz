package Sigmaz.S06_Executor.Escopos;
...
import Sigmaz.S06_Executor.Runners.Run_Def;
import Sigmaz.S06_Executor.Runners.Run_Moc;
import Sigmaz.S00_Utilitarios.AST;

import java.util.ArrayList;

public class Run_Explicit {
    ...
    public Run_Explicit(RunTime eRunTime) {
        mRunTime = eRunTime;
    ...
    public Item init_Function_Extern(AST ASTCorrente, Escopo BuscadorDeVariaveis, String eRetorne) {
        ...
        if (enc) {
            if (!algum) {
                mRunTime.errar(mLocal, "Function Explicit " + mNome + "." + ASTCorrente.getNome() + " : Argumentos incompativeis !");
            }
        } else {
            mRunTime.errar(mLocal, "Function Explicit " + mNome + "." + ASTCorrente.getNome() + " : Nao Encontrada !");
    ...
    public void Struct_Execute(AST ASTCorrente, Escopo gEscopo) {
        ...
        for (Run_Explicit mRun_Struct : mRun_Context.getRunExternContexto(gEscopo.getRefers())) {
